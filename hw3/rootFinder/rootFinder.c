...
int main(int argc, char *argv[]) {
    ...
    double guess_r_pow = guess_r;
    double error = DBL_MAX;

    while ( precision<fabs(error) ) {
        /* ... */
        for(int i = 1; i<n; i++){
            //guess_r = guess_r*guess_r;
            guess_r_pow = guess_r_pow*guess_r;
        }
        //printf("pow guess %.8f,",guess_r);
        if(guess_r_pow>x) guess_r_max=guess_r;
        else guess_r_min=guess_r;
        guess_r = (guess_r_max+guess_r_min)/2;
        guess_r_pow=guess_r;
        for(int i = 1; i<n; i++){
            //guess_r_old = guess_r_old*guess_r_old;
            guess_r_pow = guess_r_pow*guess_r;
        }
        error = guess_r_pow-x;
